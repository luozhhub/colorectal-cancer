...
setwd("/home/zhihl/Project/CRC/Chip_analysis/peak_dir_0821/promoter/")
```
---
old chip_daa: setwd("/home/zhihl/Project/CRC/Chip_analysis/peak_gene_mapping/")
...
diff_gene = function(col_1, col_2, fold_change=1 ,id_type="SYMBOL"){
  ...
  diff_week_up = unique(rownames(na.omit(week_df[week_df[,1] > fold_change & week_df[,2] < 0.05, ])))
  diff_week_down = unique(rownames(na.omit(week_df[week_df[,1] < -(fold_change) & week_df[,2] < 0.05, ])))
  
  cols <- c("SYMBOL", "ENTREZID")
  up_symbol = select(org.Mm.eg.db, keys=as.vector(diff_week_up), columns=cols, keytype="ENSEMBL")
  diff_week_up = unique(up_symbol[,"SYMBOL"])
  
  down_symbol = select(org.Mm.eg.db, keys=as.vector(diff_week_down), columns=cols, keytype="ENSEMBL")
  diff_week_down = unique(down_symbol[,"SYMBOL"])
...
#write table
write.table(commom_cancer_up , "commom_cancer_up.txt", sep="\n", quote=F, row.names=F, col.names = F, na="NA", eol="\n")
write.table(commom_cancer_down , "commom_cancer_down.txt", sep="\n", quote=F, row.names=F, col.names = F, na="NA", eol="\n")
write.table(commom_colits_up , "commom_colits_up.txt", sep="\n", quote=F, row.names=F, col.names = F, na="NA", eol="\n")
write.table(commom_colits_down , "commom_colits_down.txt", sep="\n", quote=F, row.names=F, col.names = F, na="NA", eol="\n")
```
